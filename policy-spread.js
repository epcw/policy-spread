...
// load the airport and  data together
const promises = [
...
//  console.log(" edges: " + edges.length);
  ...
  // edges are given by airport iata code (not index)
...
//  console.log(" removed: " + (old - airports.length) + " airports with low outgoing degree");
  ...
  // filter out edges that are not between airports we have leftover
  old = edges.length;
...
//  drawedges(airports, edges); //pzed commenting out until we figure out edges
...
function typeAirport(airport) {
  ...
  airport.outgoing = 0;  // eventually tracks number of outgoing edges
  airport.incoming = 0;  // eventually tracks number of incoming edges

  airport.edges = [];  // eventually tracks outgoing edges
